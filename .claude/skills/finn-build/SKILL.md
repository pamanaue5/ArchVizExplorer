...
Using the Linear connector, list issues on team `PAK` that meet every
condition:
...
- Fetch the latest default branch from `origin` and create or resume a branch
  named `PAK-NNN-short-slug`, using the issue's real identifier.
...
- `Closes PAK-NNN`, using the real Linear issue identifier
